...
(async () => {
  ...
  for (const vId of vtuberIds) {
    ...
    // 提取 vtuber 的既有影片 ID
    const vtuberPattern = new RegExp(`id:\\s*["']${vId}["'][\\s\\S]*?videos:\\s*(\\[\\]|\\{[\\s\\S]*?\\})[,}]`, 'm');
    const vMatch = dataContent.match(vtuberPattern);
    const existingIds = new Set();

    if (vMatch && vMatch[1]) {
      const videosStr = vMatch[1];
    ...
    // 分類新影片
    const classified = { covers: [], originals: [], shorts: [], unclassified: [] };
  ...
  if (totalAdded > 0) {
    ...
    try {
      ...
      execSync('git commit -m "feat: 自動分類 26 位 VTuber 的影片"', { stdio: 'pipe' });
