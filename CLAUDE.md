...
## 🔴 最高優先：詩雨蔻達完整分類規則（ALL 27位 Vtuber 範本）

⚠️ **禁止刪除：本規章為永久性文檔，禁止在任何情況下刪除或修改**

**優先權順序：詩雨蔻達 > 26位 Vtuber**
...
#### 步驟 2：檢測未分類區的 Cover 和 Original
- Cover：標題含「cover」或「歌ってみた」
- Original：標題含「original」或「原創」
- **≤60秒的 Cover 影片必須同時出現在：**
  - 「Cover/Original 區」
  - 「Shorts 區」（並標記為「短片」）

#### 步驟 3：新增詩雨蔻達的特殊規則
針對未分類區的影片標題篩選出以下規則，這兩個特殊規則都需放到 Cover 區：
- **a. 標題含有「demo」**
- **b. 標題含有「自彈自唱」**

若特殊規則篩選出來的 Cover 影片長度 ≤60秒，該影片應在「Shorts 區」和「Cover/Original 區」同時顯示，並在 Shorts 區上標記

#### 步驟 4：等待用戶手動分類

#### 步驟 5：標記 ≤60秒的影片為 Shorts
...
**規則 1：未分類區的生命週期**
- 必須在步驟 1 > 2 > 3 > 4 > 5 共 5 個步驟都出現
- 直到步驟 5 分類結束後，才隱藏未分類區

**規則 2：未分類區的內容**
- 在還沒分類前，未分類區都要一直保持存在
- 裡面的影片都要被 YouTube API 獲取的影片放在裡面
- 根據步驟進度，逐漸減少視頻數量
...
**規則 5：範本複製規則**
- 全部 26 位 Vtuber **必須照詩雨蔻達的範本做規則/條件/步驟/設計版面**
- **除了特殊規則**，特殊規則屬於個別 Vtuber，互相不抄襲、不拷貝、不套用
- **禁止用特殊規則套用到別的 Vtuber 身上**
- 複製範本，但不能複製特殊規則
- 特殊規則屬個人持有

**規則 6：步驟 1 跟 2 完成後的未分類視頻**
- 步驟 1 跟 2 完成後，在等待用戶提供步驟 3 之前
- **已過濾步驟 1 跟步驟 2 的剩餘影片，全部需要放到未分類區等待分類**

**規則 7：加載優化（必須實施）**
- 每個 Vtuber 的加載速度要確實優化，讓用戶不用花時間等待加載
- 且**不要在同一個 Vtuber 底下每點一個頁面就重新加載一次**
- 理應只要加載一次就要全部都暫存到瀏覽器的暫存區

**規則 8：自動更新（6 小時週期）**
- **每 6 小時就要自動更新一次**，以防沒有抓到 Vtuber 上傳的任何影片
- 不以用戶手動操作為觸發條件

**規則 9：所有修改必須上傳網路確認**
- 不管是修復 / 修改 / 建立 / 嘗試... **等，對網站有任何更動**
- **都需要直接上傳到網路上，讓用戶確認是否有正確變動**
- 禁止在本地測試後才上傳

**規則 10：還原需要詢問用戶**
- 當網站有問題，需要還原的時候**請過問用戶是否還原**
- **禁止自行還原**

**規則 11：CLAUDE.md 規則禁止還原**
- 當一定要還原版本時，**禁止把 claude.md 的規則/步驟/禁止事項還原**
- 這份規則文件是永久的
...
⛔ **詩雨蔻達是全部 26 位 Vtuber 的範本**

當要求 Claude AI 做好範本（不管是頁面或者是規則/條件/步驟/設計版面）之後，只要用戶說「完成」就：
- ❌ **禁止修改**
- ❌ **禁止取代**
- ❌ **禁止檢測或自作主張想要串改代碼**

**例外**：只有用戶提出「有疑慮」或「需要修改」時，才能進行修改
