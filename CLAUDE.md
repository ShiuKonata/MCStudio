...
## 🚨 最高優先規則（2026-06-09 新增）：禁止轉嫁責任

**從 2026-06-09 起，永遠遵守以下規則：**

- ✅ **禁止要求用戶執行任何 Claude AI 不想自己做的事情**
- ✅ 不能因為技術限制就把責任推給用戶
- ✅ 必須自己找辦法解決問題（尋找替代方案、調整方法、或坦誠承認無法完成）
- ✅ 如果真的無法完成，要清楚說明原因，而非要求用戶代勞

**背景：** 用戶發現 Claude 要求他們在瀏覽器執行 YouTube API 代碼，但這是 Claude 應該自己解決的技術問題。
...
### 🟡 內層分類規則：詩雨蔻達影片分類規章（由外層工作流程第七步呼叫）
...
## 🔵 外層工作流程（2026-06-10 新增）：每位 VTuber 的執行步驟

**此為外層工作執行框架，每位 VTuber 都必須完整執行以下 10 步驟，順序不可更改，步驟不可省略。**
...
**第七步**：針對 Unclassified（未分類）頁面，啟動本文件的【內層分類規則】（詩雨蔻達影片分類規章）
**第八步**：再次確認分類好的影片區是否數量正確，檢測原創曲、cover、一般影片、vlog、廣告、初配信、工商、未分類
**第九步**：完成後上傳
**第十步**：回報自己工作狀態，是否還記得這十個步驟，一字一句不能遺漏回覆給用戶

> ⚠️ **第七步說明**：「啟動 CLAUDE.md 的影片分類步驟」指的是下方的【內層分類規則：詩雨蔻達影片分類規章】。外層流程負責取得影片數據，內層規則負責分類邏輯，兩者為不同層級，互不衝突。

---

## 🟡 內層分類規則（詩雨蔻達影片分類規章）— 由外層第七步呼叫
